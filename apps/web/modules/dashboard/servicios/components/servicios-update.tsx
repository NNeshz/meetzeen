...
import { useUpdateServicioMutation } from "@/modules/dashboard/servicios/hooks/useServicios"
...
export function ServicioUpdate({ 
  id, 
  name, 
  duration, 
  price, 
  ...
  const updateServicioMutation = useUpdateServicioMutation()
  const { data: categoriasData, isLoading: categoriasLoading } = useCategoriasQuery()

  const onSubmit = form.handleSubmit(async (data) => {
    try {
      await updateServicioMutation.mutateAsync({
        id,
        name: data.name,
    ...
    <Form {...form}>
      <form onSubmit={onSubmit} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Actualiza el nombre descriptivo de tu servicio.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select 
                ...
                value={field.value}
                disabled={updateServicioMutation.isPending || categoriasLoading}
              >
                <FormControl>
        ...
        <Button 
          ...
          disabled={updateServicioMutation.isPending || categoriasLoading}
        >
          {updateServicioMutation.isPending ? "Actualizando..." : "Actualizar Servicio"}
        </Button>
      </form>
    </Form>
  )
