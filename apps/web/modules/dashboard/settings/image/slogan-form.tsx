...
import { useUpdateCompanySlogan } from "@/modules/dashboard/settings/hooks/useNegocio";
...
export function SloganForm({ slogan }: { slogan: string }) {
  ...
  const { mutateAsync, isPending } = useUpdateCompanySlogan();
  const initialSlogan = slogan;
  const isUnchanged = form.watch("slogan") === initialSlogan;

  // 2. Define a submit handler.
  function onSubmit(values: z.infer<typeof formSchema>) {
    return mutateAsync(values.slogan);
  }
